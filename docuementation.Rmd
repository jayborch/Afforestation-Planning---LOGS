...
We created a spatial resistance model to estimate how accessible different areas are for afforestation activities, based on the combined effects of land cover and terrain. Land-cover resistance values were assigned using the national Vistlendi habitat classes, with higher costs given to difficult environments such as wetlands, scree, lava fields, and freshwater, and lower costs to more accessible surfaces like grasslands and anthropogenic areas. To account for slope-related travel difficulty, we applied a logistic resistance function to a 10-m DEM, reflecting the increased effort and reduced mobility of field crews and lightweight vehicles on steeper ground. 
...
# plot with natural breaks using a viridis palette
plot(out_accum,
     breaks = breaks,
     col = viridis(n_classes),
     main = "Cost Accumulation for traversing the landscape from a road")
